...
def generate_html():
    ...
    test_json = json.load(open("/localhome/qiruiw/research/RoomFormer/data/stru3d/annotations/test.json"))
    
    with open(os.path.join(out_path, "s3d.html"), "w") as f:
        write_header(f)
        f.write("<table>\n")
        ...
        for i, image in enumerate(test_json["images"]):
            ...
            pc_topdown = os.path.join(f"roomformer/s3d/{scene_id}/topdown.png")
            density_map = os.path.join(f"roomformer/data/stru3d/test/{scene_id}.png")
            gt_floorplan = os.path.join(f"roomformer/checkpoints/eval_stru3d/{img_id}_gt.png")
            pred_floorplan = os.path.join(f"roomformer/checkpoints/eval_stru3d/{img_id}_pred_floorplan.png")
            gt_sem_floorplan = os.path.join(f"roomformer/checkpoints/eval_stru3d_sem_rich/{img_id}_sem_rich_gt.png")
            pred_sem_floorplan = os.path.join(f"roomformer/checkpoints/eval_stru3d_sem_rich/{img_id}_sem_rich_pred.png")
